...
MODEL_DIR = "models"
RESULTS_DIR = "results/rf_walk_stairs/open_set"
...
def main():
    console.print(Panel.fit("[bold cyan]OPEN SET PROTOCOL: RANDOM FOREST (Walk/Stairs)[/bold cyan]"))
    ...
    if USE_ALL_VALID_AS_KNOWN:
        ...
        params_name = "best_params_rf_walk_stairs_closed_set.json"
    else:
        model_name = "rf_walk_stairs_open_set.joblib"
        params_name = "best_params_rf_walk_stairs_open_set.json"
    ...
    plt.plot(FAR, DIR, label='Watchlist ROC', color='b', linewidth=2)
    plt.plot(FAR[eer_index], DIR[eer_index], 'ro', markersize=8, label=f'EER Point ({eer_value*100:.1f}%)')
    plt.title('Watchlist ROC (Open Set) - Random Forest Walk/Stairs')
    plt.xlabel('False Alarm Rate (FAR)')
    plt.ylabel('Detect & Identify Rate (DIR)')
    plt.xlim([-0.05, 1.05]); plt.ylim([-0.05, 1.05])
    plt.grid(True, linestyle='--', alpha=0.6); plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, f'roc_watchlist_rf{suffix}.png'), dpi=300)
    plt.close()

    with open(os.path.join(RESULTS_DIR, f"open_set_report{suffix}.txt"), "w") as f:
        f.write("========================================================\n")
        f.write(" OPEN SET WATCHLIST REPORT: Random Forest Walk/Stairs\n")
        f.write("========================================================\n")
