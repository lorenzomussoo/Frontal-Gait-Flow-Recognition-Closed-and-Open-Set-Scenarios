...
RESULTS_DIR = "results/alt_models/mlp_walk_stairs/no_flip/closed_set"
MODEL_NAME = "mlp_walk_stairs_closed_set_no_flip.joblib"
PARAMS_FILE = os.path.join(MODEL_DIR, 'best_params_mlp_walk_stairs_closed_set_no_flip.json')
...
def load_data():
    console.print(Panel.fit(f"[bold yellow]Loading Walk/Stairs Dataset for MLP: {FEATURES_ROOT}[/bold yellow]"))
...
def train_and_evaluate():
    ...
    if SAVE_RESULTS:
        with open(os.path.join(RESULTS_DIR, "metrics_report.txt"), "w") as f:
            f.write("=== FINAL REPORT: MLP (WALK/STAIRS) ===\n\n--- CLASSIFICATION REPORT ---\n")
            f.write(report_text + "\n\n--- CUMULATIVE MATCH SCORE (CMS) ---\n")
            for k in [1, 2, 3, 4, 5, 10]:
                if k <= G: f.write(f"Rank-{k}: {cms[k-1]:.2f}%\n")

        plt.figure(figsize=(10, 6))
        plt.plot(range(1, G + 1), cms, marker='o', linestyle='-', color='orange', linewidth=2)
        plt.title('Cumulative Match Characteristic (CMC) - MLP Walk/Stairs')
        ...
        plt.title('Confusion Matrix - MLP Walk/Stairs')
