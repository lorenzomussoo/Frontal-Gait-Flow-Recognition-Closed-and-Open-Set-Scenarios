...
RESULTS_DIR = "results/alt_models/svm_walk_stairs/masked_open_set"
...
def build_or_train_pipeline(X_train, y_train):
    params_path = os.path.join(MODEL_DIR, "best_params_svm_walk_stairs.json")
    
    if PERFORM_GRID_SEARCH:
        console.print("[yellow]Running Grid Search Optimization for SVM...[/yellow]")
        pipeline = Pipeline([
        ...
        if os.path.exists(params_path):
            ...
            model_params = {k: v for k, v in loaded_params.items() if k.startswith('svm__')}
            base_pipeline.set_params(scaler=StandardScaler(), **model_params)
        else:
...
def run_analysis(valid_subs, incomplete_subs):
    ...
    plt.plot(FAR_array, DIR_array, label=f'Watchlist ROC (AUC = {roc_auc:.4f})', color='indigo', linewidth=2.5)
    plt.plot(eer_value, dir_at_eer, 'ro', markersize=8, label=f'EER Point ({eer_value*100:.2f}%)')
    plt.title('Masked Open Set ROC - SVM (Walk/Stairs)', fontsize=14, pad=15)
    plt.xlabel('False Alarm Rate (FAR)', fontsize=12)
    plt.ylabel('Detect & Identify Rate (DIR)', fontsize=12)
    plt.xlim([-0.02, 1.02]); plt.ylim([-0.02, 1.02])
    plt.grid(True, linestyle='--', alpha=0.6); plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, 'roc_svm_walk_stairs.png'), dpi=300)
    plt.close()
    ...
    plt.title('Score Distribution - SVM (Walk/Stairs)', fontsize=14, pad=15)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, 'dist_svm_walk_stairs.png'), dpi=300)
    plt.close()
    
    report_path = os.path.join(RESULTS_DIR, "svm_walk_stairs_comprehensive_report.txt")
    with open(report_path, "w") as f:
        f.write("==================================================================\n")
        f.write(" COMPREHENSIVE OPEN SET REPORT - SVM (WALK/STAIRS)\n")
        f.write("==================================================================\n\n")
...
def main():
    console.print(Panel.fit("[bold magenta]COMPREHENSIVE OPEN SET ANALYSIS - SVM WALK/STAIRS[/bold magenta]"))
