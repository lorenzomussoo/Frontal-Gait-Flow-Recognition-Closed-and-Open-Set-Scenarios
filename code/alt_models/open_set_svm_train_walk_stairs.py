...
RESULTS_DIR = "results/alt_models/svm_walk_stairs/open_set"
...
def main():
    console.print(Panel.fit("[bold cyan]OPEN SET PROTOCOL: SVM (Walk/Stairs)[/bold cyan]"))
    ...
    if USE_ALL_VALID_AS_KNOWN:
        ...
        params_name = "best_params_svm_walk_stairs_closed_set.json"
    else:
        model_name = "svm_walk_stairs_open_set.joblib"
        params_name = "best_params_svm_walk_stairs_open_set.json"
    ...
    plt.plot(FAR, DIR, label='Watchlist ROC', color='indigo', linewidth=2)
    plt.plot(eer_value, dir_at_eer, 'ro', markersize=8, label=f'EER Point ({eer_value*100:.1f}%)')
    plt.title('Watchlist ROC (Open Set) - SVM Walk/Stairs')
    ...
    with open(os.path.join(RESULTS_DIR, f"open_set_report{suffix}.txt"), "w") as f:
        f.write("========================================================\n")
        f.write(" OPEN SET WATCHLIST REPORT: SVM Walk/Stairs\n")
        f.write("========================================================\n")
