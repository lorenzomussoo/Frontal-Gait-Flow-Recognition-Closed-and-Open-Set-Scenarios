...
RESULTS_DIR = "results/alt_models/hgb_slope/open_set"
...
def main():
    console.print(Panel.fit("[bold magenta]OPEN SET PROTOCOL: HGB (Slope)[/bold magenta]"))
    ...
    model_name = "hgb_slope_closed_set.joblib" if USE_ALL_VALID_AS_KNOWN else "hgb_slope_open_set.joblib"
    params_name = "best_params_hgb_slope_closed_set.json" if USE_ALL_VALID_AS_KNOWN else "best_params_hgb_slope_open_set.json"
    ...
    if not is_grid_search and os.path.exists(model_path):
        ...
        if is_grid_search:
            ...
            param_grid = [{'pca': ['passthrough',PCA(n_components=0.95), PCA(n_components=0.99)], 'hgb__learning_rate': [0.05, 0.1], 'hgb__max_iter': [100, 200], 'hgb__max_depth': [5, 10, 20]}]
    ...
    plt.title('Watchlist ROC (Open Set) - HGB Slope')
    ...
    with open(os.path.join(RESULTS_DIR, f"open_set_report{suffix}.txt"), "w") as f:
        f.write("========================================================\n")
        f.write(" OPEN SET WATCHLIST REPORT: HGB Slope\n")
        f.write("========================================================\n")
