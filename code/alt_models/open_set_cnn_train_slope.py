...
RESULTS_DIR = "results/alt_models/cnn_slope/open_set"
...
class GaitCNN1D_IMU(nn.Module):
    ...
    def forward(self, x):
        x = self.features(x)
        if x.device.type == 'mps': x = nn.functional.adaptive_avg_pool1d(x.cpu(), 50).to('mps')
...
def main():
    console.print(Panel.fit("[bold magenta]OPEN SET PROTOCOL: CNN 1D (Slope)[/bold magenta]"))
    ...
    model = GaitCNN1D_IMU(num_classes=len(known)).to(device)

    model_name = "cnn_slope_closed_set.pth" if USE_ALL_VALID_AS_KNOWN else "cnn_slope_open_set.pth"
    ...
    plt.title('Watchlist ROC (Open Set) - CNN Slope')
    ...
    with open(os.path.join(RESULTS_DIR, f"open_set_report{suffix}.txt"), "w") as f:
        f.write("========================================================\n")
        f.write(" OPEN SET WATCHLIST REPORT: CNN Slope\n")
        f.write("========================================================\n")
