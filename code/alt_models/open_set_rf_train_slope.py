...
RESULTS_DIR = "results/alt_models/rf_slope/open_set"
...
def main():
    console.print(Panel.fit("[bold magenta]OPEN SET PROTOCOL: Random Forest (Slope)[/bold magenta]"))
    ...
    if USE_ALL_VALID_AS_KNOWN:
        ...
        params_name = "best_params_rf_slope_closed_set.json"
    else:
        model_name = "rf_slope_open_set.joblib"
        params_name = "best_params_rf_slope_open_set.json"
    ...
    plt.plot(FAR, DIR, label='Watchlist ROC', color='blue', linewidth=2)
    plt.plot(eer_value, dir_at_eer, 'ro', markersize=8, label=f'EER Point ({eer_value*100:.1f}%)')
    plt.title('Watchlist ROC (Open Set) - RF Slope')
    ...
    with open(os.path.join(RESULTS_DIR, f"open_set_report{suffix}.txt"), "w") as f:
        f.write("========================================================\n")
        f.write("         OPEN SET WATCHLIST REPORT: RF Slope\n")
        f.write("========================================================\n")
