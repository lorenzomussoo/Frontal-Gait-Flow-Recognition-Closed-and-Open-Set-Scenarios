...
MODEL_DIR = "models"
RESULTS_DIR = "results/rf_slope/closed_set"
MODEL_NAME = "rf_slope_closed_set.joblib"
PARAMS_FILE = os.path.join(MODEL_DIR, 'best_params_rf_slope_closed_set.json')
...
def load_data():
    console.print(Panel.fit(f"[bold magenta]Loading SLOPE Dataset for RANDOM FOREST: {FEATURES_ROOT}[/bold magenta]"))
            ...
            for root, dirs, files in os.walk(run_dir):
                ...
                for f in sorted(files):
                    if f.endswith('.npy') and 'flip' not in f and not f.startswith('._'):
                        slope_count += 1
            if slope_count < 6:
                console.print(f"[yellow]Excluded {subj}: in {run_name} has only {slope_count} Slope acquisitions.[/yellow]")
                is_complete = False
                break
...
def train_and_evaluate():
    ...
    if PERFORM_GRID_SEARCH:
        ...
        param_grid = [
            {'scaler': ['passthrough'], 'pca': ['passthrough', PCA(n_components=0.95)], 
             'rf__n_estimators': [100, 200, 500], 'rf__max_depth': [None, 30], 'rf__min_samples_split': [2, 5]}
        ]
        ...
        if SAVE_RESULTS:
            ...
            for k, v in search.best_params_.items():
                if k == 'pca' and isinstance(v, PCA): best_params_serializable['pca_status'] = 'active'; best_params_serializable['pca__n_components'] = v.n_components
                elif k == 'pca' and v == 'passthrough': best_params_serializable['pca_status'] = 'passthrough'
                elif k != 'scaler': best_params_serializable[k] = v

            with open(PARAMS_FILE, 'w') as f: json.dump(best_params_serializable, f, indent=4)
        ...
        if os.path.exists(PARAMS_FILE):
            ...
            if loaded_params.get('pca_status') == 'passthrough' or loaded_params.get('pca') == 'passthrough': base_pipeline.set_params(pca='passthrough')
    ...
    if SAVE_RESULTS:
        with open(os.path.join(RESULTS_DIR, "metrics_report.txt"), "w") as f:
            f.write("=== FINAL REPORT: RANDOM FOREST (SLOPE IMU) ===\n\n")
        ...
        plt.title('Cumulative Match Characteristic (CMC) - RF Slope')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(RESULTS_DIR, 'cmc_curve.png'), dpi=300); plt.close()

        cm = confusion_matrix(y_test_str, y_pred_str_out, labels=classes)
        plt.figure(figsize=(14, 12))
        sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=classes, yticklabels=classes)
        plt.title('Confusion Matrix - RF Slope')
