...
MODEL_DIR = "models"
RESULTS_DIR = "results/cnn_slope/closed_set"
...
def load_data():
    console.print(Panel.fit(f"[bold magenta]Loading SLOPE Dataset for PyTorch CNN: {FEATURES_ROOT}[/bold magenta]"))
...
class GaitCNN1D_IMU(nn.Module):
    def __init__(self, num_classes):
        super(GaitCNN1D_IMU, self).__init__()
        self.features = nn.Sequential(
            nn.Conv1d(1, 16, kernel_size=15, stride=3, padding=7), nn.ReLU(), nn.BatchNorm1d(16), nn.MaxPool1d(4),
        ...
        self.classifier = nn.Sequential(
            nn.Flatten(), nn.Linear(64 * 50, 128), nn.ReLU(), nn.Dropout(0.5), nn.Linear(128, num_classes)
        )

    def forward(self, x):
        x = self.features(x)
        if x.device.type == 'mps':
            x = nn.functional.adaptive_avg_pool1d(x.cpu(), 50).to('mps')
        else: x = nn.functional.adaptive_avg_pool1d(x, 50)
...
def train_and_evaluate():
    ...
    model = GaitCNN1D_IMU(num_classes=len(classes)).to(device)
    optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE, weight_decay=1e-3)
    criterion = nn.CrossEntropyLoss()
    
    console.print(Panel("[bold green]Starting Training Loop (Slope IMU)...[/bold green]"))
    ...
    if SAVE_RESULTS:
        with open(os.path.join(RESULTS_DIR, "metrics_report.txt"), "w") as f:
            f.write("=== FINAL REPORT: CNN (SLOPE) ===\n\n--- CLASSIFICATION REPORT ---\n")
            f.write(report_text + "\n\n--- CUMULATIVE MATCH SCORE (CMS) ---\n")
            for k in [1, 2, 3, 4, 5, 10]:
                if k <= G: f.write(f"Rank-{k}: {cms[k-1]:.2f}%\n")

        plt.figure(figsize=(10, 6))
        plt.plot(range(1, G + 1), cms, marker='s', linestyle='-', color='teal', linewidth=2)
        plt.title('Cumulative Match Characteristic (CMC) - CNN Slope')
        ...
        plt.title('Confusion Matrix - CNN Slope')
        ...
        torch.save(model.state_dict(), os.path.join(MODEL_DIR, "cnn_slope.pth"))
