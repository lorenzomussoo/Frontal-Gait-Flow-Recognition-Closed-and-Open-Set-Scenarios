...
MODEL_DIR = "models"
RESULTS_DIR = "results/hgb_slope/closed_set"
MODEL_NAME = "hgb_slope_closed_set.joblib" 
PARAMS_FILE = os.path.join(MODEL_DIR, 'best_params_hgb_slope_closed_set.json')
...
def load_data():
    console.print(Panel.fit(f"[bold magenta]Loading SLOPE Dataset for GRADIENT BOOSTING: {FEATURES_ROOT}[/bold magenta]"))
    ...
    for subj in candidate_subjects:
        ...
        for run_name in ["FirstRun", "SecondRun", "ThirdRun"]:
            ...
            if slope_count < 6:
                console.print(f"[yellow]Excluded {subj}: {run_name} only has {slope_count} Slope acquisitions (6 required).[/yellow]")
                is_complete = False
                break
...
def train_and_evaluate():
    ...
    if PERFORM_GRID_SEARCH:
        console.print(Panel.fit("[bold magenta]Starting HGB Grid Search for Slope...[/bold magenta]"))
        param_grid = [
            {
                'pca': ['passthrough', PCA(n_components=100), PCA(n_components=50), PCA(n_components=0.95), PCA(n_components=0.99)], 
        ...
        console.print(Panel.fit("[bold magenta]Training HGB with saved/default parameters...[/bold magenta]"))
        if os.path.exists(PARAMS_FILE):
            ...
            base_pipeline.set_params(**hgb_params)
        else:
            base_pipeline.set_params(pca='passthrough', hgb__learning_rate=0.1, hgb__max_iter=200) 
    ...
    if SAVE_RESULTS:
        with open(os.path.join(RESULTS_DIR, "metrics_report.txt"), "w") as f:
            f.write("=== FINAL REPORT: HGB (SLOPE IMU) ===\n\n")
        ...
        plt.plot(range(1, G + 1), cms, marker='s', linestyle='-', color='purple', linewidth=2)
        plt.title('Cumulative Match Characteristic (CMC) - HGB Slope (IMU)')
        plt.xlabel('Rank')
        ...
        plt.title('Confusion Matrix - HGB Slope (IMU)')
