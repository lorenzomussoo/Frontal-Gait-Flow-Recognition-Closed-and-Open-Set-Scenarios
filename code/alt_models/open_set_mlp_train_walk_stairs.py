...
RESULTS_DIR = "results/alt_models/mlp_walk_stairs/masked_open_set"
...
def build_or_train_pipeline(X_train, y_train):
    params_path = os.path.join(MODEL_DIR, "best_params_mlp_walk_stairs.json")
    
    if PERFORM_GRID_SEARCH:
        console.print("[yellow]Running Grid Search Optimization for MLP...[/yellow]")
        pipeline = Pipeline([
        ...
        param_grid = [
            {
                ...
                'mlp__hidden_layer_sizes': [(100,), (256, 128), (128, 64)], 
                'mlp__alpha': [0.0001, 0.01, 1.0, 10.0]
            }
        ]
        ...
        if os.path.exists(params_path):
            ...
            model_params = {k: v for k, v in loaded_params.items() if k.startswith('mlp__')}
            base_pipeline.set_params(**model_params)
        else:
            console.print("[red]JSON non trovato! Uso i parametri di default.[/red]")
            base_pipeline.set_params(pca='passthrough', mlp__hidden_layer_sizes=(128, 64), mlp__alpha=10.0)
...
def run_analysis(valid_subs, incomplete_subs):
    ...
    plt.plot(FAR_array, DIR_array, label=f'Watchlist ROC (AUC = {roc_auc:.4f})', color='orange', linewidth=2.5)
    plt.plot(eer_value, dir_at_eer, 'ro', markersize=8, label=f'EER Point ({eer_value*100:.2f}%)')
    plt.title('Masked Open Set ROC - MLP (Walk/Stairs)', fontsize=14, pad=15)
    plt.xlabel('False Alarm Rate (FAR)', fontsize=12)
    plt.ylabel('Detect & Identify Rate (DIR)', fontsize=12)
    plt.xlim([-0.02, 1.02]); plt.ylim([-0.02, 1.02])
    plt.grid(True, linestyle='--', alpha=0.6); plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, 'roc_mlp_walk_stairs.png'), dpi=300)
    plt.close()
    ...
    plt.title('Score Distribution - MLP (Walk/Stairs)', fontsize=14, pad=15)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, 'dist_mlp_walk_stairs.png'), dpi=300)
    plt.close()
    
    report_path = os.path.join(RESULTS_DIR, "mlp_walk_stairs_comprehensive_report.txt")
    with open(report_path, "w") as f:
        f.write("==================================================================\n")
        f.write(" COMPREHENSIVE OPEN SET REPORT - MLP (WALK/STAIRS)\n")
        f.write("==================================================================\n\n")
...
def main():
    console.print(Panel.fit("[bold magenta]COMPREHENSIVE OPEN SET ANALYSIS - MLP WALK/STAIRS[/bold magenta]"))
