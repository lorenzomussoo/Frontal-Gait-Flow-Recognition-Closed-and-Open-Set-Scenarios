...
RESULTS_DIR = "results/alt_models/mlp_walk_stairs/open_set"
...
def main():
    console.print(Panel.fit("[bold cyan]OPEN SET PROTOCOL: MLP (Walk/Stairs)[/bold cyan]"))
    ...
    model_name = "mlp_walk_stairs_closed_set.joblib" if USE_ALL_VALID_AS_KNOWN else "mlp_walk_stairs_open_set.joblib"
    params_name = "best_params_mlp_walk_stairs_closed_set.json" if USE_ALL_VALID_AS_KNOWN else "best_params_mlp_walk_stairs_open_set.json"
    ...
    plt.plot(FAR, DIR, label='Watchlist ROC', color='orange', linewidth=2)
    plt.plot(eer_value, dir_at_eer, 'ro', markersize=8, label=f'EER Point ({eer_value*100:.1f}%)')
    plt.title('Watchlist ROC (Open Set) - MLP Walk/Stairs')
    ...
    plt.tight_layout(); plt.savefig(os.path.join(RESULTS_DIR, f'roc_watchlist_mlp{suffix}.png'), dpi=300); plt.close()

    with open(os.path.join(RESULTS_DIR, f"open_set_report{suffix}.txt"), "w") as f:
        f.write("========================================================\n")
        f.write(" OPEN SET WATCHLIST REPORT: MLP Walk/Stairs\n")
        f.write("========================================================\n")
